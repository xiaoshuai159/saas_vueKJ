...
export default {
  ...
  methods: {
    ...
    async ipxuan() {
      ...
      if (res.code == 200) {
        if (res.data.length > 0) {
          ...
          this.visits = visitsleft1.reverse();
          this.sourceip = sourceipleft1.reverse();
          setTimeout(() => {
            this.drawLine();
            // this.loading = false;
          }, 500);
    ...
    async urlxuan() {
      ...
      if (res.code == 200) {
        if (res.data.length > 0) {
          ...
          this.urlvisits = urlvisitsright1.reverse();
          this.url = urlright1.reverse();
          // this.loading = false;
          setTimeout(() => {
            this.drawLine1();
            // this.loading = false;
    ...
    drawLinequ1() {
      ...
      let myChart = this.$echarts.init(bar_qx);
      myChart.setOption(this.setOptionqu1());
    },
    ...
    setOption2() {
      let option = {
        title: {
          // text: "成都市访问量历史变化趋势",
          text: this.newcity == true ? this.title : this.cityName + this.title1,
          textStyle: {
            //xin
            fontSize: 18,
            color: "#fff", //xin
    ...
    mineMap() {
      ...
      var mapOption = {
        ...
        series: [
          ...
          //       if (v <= this.max / 4) {
          //         return [20, 30];
          //       } else if (v <= (this.max / 4) * 2 && v > this.max / 4) {
          //         return [20, 60];
          //       } else if (v <= (this.max / 4) * 3 && v > (this.max / 4) * 2) {
          //         return [20, 90];
          //       } else {
          //         return [20, 120];
          //       }
          ...
          // console.log(pa);
          //       if (pa < this.max / 4) {
          //         return `image://${require("@/assets/img/taishi/lower.png")}`;
          //       } else if (pa < (this.max / 4) * 2) {
          //         return `image://${require("@/assets/img/taishi/low.png")}`;
          //       } else if (pa < (this.max / 4) * 3) {
          //         return `image://${require("@/assets/img/taishi/high.png")}`;
          //       } else {
          //         return `image://${require("@/assets/img/taishi/higher.png")}`;
          //       }
          //     },
          //   },
        ],
